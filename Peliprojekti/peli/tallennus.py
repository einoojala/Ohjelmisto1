...
# Tuodaan kaikki pelin huoneet, jotta tallennettu sijainti
# voidaan yhdistää oikeaan Huone-olioon.
from peli.huoneet import tyohuone, kirjasto, keittio, olohuone, ruokasali

# Tuodaan kaikki pelissä kerättävät esineet, jotta tallennettu
# esineen nimi voidaan yhdistää oikeaan Esine-olioon.
...
def tallenna_peli(pelaaja):
    ...
    # "w" tarkoittaa kirjoitustilaa ja korvaa vanhan tallennuksen.
    with open("data/save.txt", "w", encoding="utf-8") as tiedosto:
        ...
        # Tarkistetaan ensin, että pelaajalla on jokin sijainti.
        if pelaaja.sijainti is not None:
            tiedosto.write(f"sijainti:{pelaaja.sijainti.nimi}\n")
        else:
            # Jos pelaaja ei ole vielä ollut missään huoneessa,
            # tallennetaan sijainnin kohdalle tyhjä arvo.
            tiedosto.write("sijainti:\n")

        # Tallennetaan pelaajan inventaario.
        # Inventaario sisältää Esine-olioita, mutta tiedostoon
        # tallennetaan vain esineiden nimet.
        esineet = []
        ...
        # Esimerkiksi:
        # ["Tutkimuspaperi", "Teekuppi"]
        # muuttuu muotoon:
        # "Tutkimuspaperi,Teekuppi"
        tiedosto.write(f"inventaario:{','.join(esineet)}\n")

        # Tallennetaan kaikki pelaajan löytämät vihjeet.
        # Vihjeet erotetaan toisistaan |-merkillä.
        tiedosto.write(f"vihjeet:{'|'.join(pelaaja.vihjeet)}\n")

        # Tallennetaan tieto siitä, onko PIN-koodi ratkaistu.
        # Arvo tallentuu tekstinä True tai False.
...
def lataa_peli():
    ...
    # Luodaan tyhjä sanakirja, johon tallennustiedoston
    # avaimet ja arvot voidaan sijoittaa.
    tiedot = {}
    ...
    for rivi in rivit:
        ...
        # split(":", 1) jakaa rivin kahteen osaan:
        # avaimeen ja arvoon.
        #
        # Esimerkiksi:
        # "nimi:Maija"
    ...
    # int() muuttaa tiedostosta luetun iän merkkijonosta kokonaisluvuksi.
    pelaaja = Pelaaja(
        tiedot["nimi"],
        int(tiedot["ika"])
    )

    # Sanakirjan avulla tallennettu huoneen nimi yhdistetään
    # oikeaan Huone-olioon.
    huoneet = {
        "Työhuone": tyohuone,
        "Kirjasto": kirjasto,
        "Keittiö": keittio,
    ...
    # Jos löytyy, pelaajan sijainniksi asetetaan kyseinen Huone-olio.
    if tiedot["sijainti"] in huoneet:
        pelaaja.sijainti = huoneet[tiedot["sijainti"]]

    # Sanakirja, jonka avulla tallennettu esineen nimi
    # voidaan yhdistää oikeaan Esine-olioon.
    kaikki_esineet = {
    ...
    # Jos inventaariossa on tallennettuja esineitä,
    # palautetaan ne pelaajan inventaarioon.
    if tiedot["inventaario"]:

        # split(",") jakaa tallennetun merkkijonon takaisin
        # yksittäisiksi esineiden nimiksi.
        #
        # Esimerkiksi:
        # "Tutkimuspaperi,Teekuppi"
        # muuttuu listaksi:
        ...
        # Käydään kaikki tallennetut esineiden nimet läpi.
    ...
    # Jos pelaajalla on tallennettuja vihjeitä,
    # palautetaan ne pelaajan vihjelistaan.
    if tiedot["vihjeet"]:

        # split("|") jakaa tallennetun tekstin takaisin
        # yksittäisiksi vihjeiksi.
    ...
    # Pelaajan mukana olevat esineet täytyy poistaa huoneista.
    # Muuten ladatussa pelissä sama esine voisi löytyä
    # sekä pelaajan inventaariosta että alkuperäisestä huoneesta.
    for huone in huoneet.values():

        # [:] tekee huoneen esineistä kopion, jota voidaan käydä läpi
        # samalla kun alkuperäisestä listasta poistetaan esineitä.
        for esine in huone.esineet[:]:

            # Jos esine on pelaajan inventaariossa,
            # se ei voi enää olla huoneessa.
