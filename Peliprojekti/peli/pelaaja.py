...
class Pelaaja:
    ...
    def __init__(self, nimi, ika):
        ...
        # Lista pelaajan mukana olevista esineistä.
        # Esineitä lisätään listaan pelin aikana.
        self.inventaario = []
        ...
        # Aluksi pelaaja ei ole vielä missään huoneessa.
        self.sijainti = None

        # False tarkoittaa, että tietokoneen PIN-koodia ei ole vielä ratkaistu.
    ...
    def nayta_inventaario(self):
        ...
        # Valikko pysyy auki, kunnes pelaaja valitsee x tai
        # inventaario on tyhjä.
        while True:
            print("\n--- INVENTAARIO ---")

            # Jos inventaariossa ei ole esineitä,
            # ilmoitetaan siitä ja poistutaan metodista.
            if len(self.inventaario) == 0:
                print("Inventaario on tyhjä.")
                return

            # enumerate() antaa jokaiselle esineelle järjestysnumeron.
            # Numero alkaa arvosta 1, jotta pelaajan on helpompi valita esine.
            ...
            # Jos pelaaja valitsee x, palataan takaisin päävalikkoon.
            if valinta.lower() == "x":
                return

            # Tarkistetaan, että pelaajan antama valinta on numero.
            # Jos se ei ole numero, palataan while-silmukan alkuun.
            if not valinta.isdigit():
                print("Anna numero.")
                continue

            # Muutetaan käyttäjän antama merkkijono kokonaisluvuksi,
            # jotta sitä voidaan käyttää listan indeksinä.
            numero = int(valinta)

            # Tarkistetaan, että annettu numero vastaa
            # jotain inventaarion esinettä.
            if 1 <= numero <= len(self.inventaario):

                # Haetaan valittu esine inventaariosta.
                # Listan indeksit alkavat nollasta, joten käyttäjän
                # antamasta numerosta vähennetään yksi.
                esine = self.inventaario[numero - 1]

                # Kutsutaan esineen tutki()-metodia.
                # self tarkoittaa tässä nykyistä Pelaaja-oliota,
                # jotta esine voi lisätä löytyvän vihjeen pelaajalle.
                esine.tutki(self)

            else:
                # Ilmoitetaan, jos käyttäjän antama numero
                # ei vastaa mitään inventaarion esinettä.
    ...
    def nayta_vihjeet(self):
        ...
        # Jos pelaaja ei ole löytänyt vielä yhtään vihjettä,
        # näytetään siitä ilmoitus.
        if len(self.vihjeet) == 0:
            print("Et ole vielä löytänyt vihjeitä.")

        else:
            # Käydään kaikki löydetyt vihjeet läpi
