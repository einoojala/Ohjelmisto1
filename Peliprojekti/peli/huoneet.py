# Tuodaan huoneissa olevat esineet esine.py-tiedostosta.
# Näin jokaiselle huoneelle voidaan määrittää, mitä esineitä
# pelaaja voi kyseisestä huoneesta löytää ja kerätä.
...
# Jokaisella huoneella on nimi, kuvaus, tutkittaessa löytyvä vihje
# sekä lista huoneessa olevista esineistä.
class Huone:

    # Alustetaan uuden huoneen tiedot.
    ...
    # Kun huonetta tulostetaan print()-komennolla, pelaajalle näytetään
    # huoneen nimi ja kuvaus.
    def __str__(self):
        return f"\n---- {self.nimi.upper()} ----\n{self.kuvaus}"

    # Metodia käytetään, kun pelaaja haluaa tutkia huonetta tarkemmin.
    # Pelaaja voi joko tutkia huoneen tai palata takaisin.
    def tutki_tarkemmin(self, pelaaja):

        # while True pitää kysymyksen näkyvissä, jos pelaaja
        # antaa virheellisen vastauksen.
        while True:
            vastaus = input(
                "\nHaluatko tutkia huonetta tarkemmin? (kyllä/ei): "
            ).lower()
            ...
            if vastaus == "kyllä":
                ...
                # Huoneesta löytyvä vihje lisätään pelaajan
                # löytämien vihjeiden listaan.
                ...
                # Jos vastaus ei ole "kyllä" tai "ei",
                # kysytään vastaus uudelleen.
...
'400 x 25. Muista kokonaisteho ja käytä viimeisiä numeroita.'""",
...
# Keittiössä pelaaja voi löytää Sofian avainkortin ja teekupin.
# Näistä saadaan myös murhaan liittyviä vihjeitä.
keittio = Huone(
    "Keittiö",
...
Seinäkello näyttää aikaa 22.17.""",

    [avainkortti, kuppi]
)


# Olohuoneesta löytyy muistilappu.
# Huoneen tarkemmassa tutkimisessa saadaan PIN-koodin ensimmäinen numero.
olohuone = Huone(
    "Olohuone",
...
kuusi vuotta sitten kaikki muuttui""",

    [muistilappu]
)


# Ruokasalista löytyvä lappu antaa PIN-koodin seuraavan osan.
# Pelaajan täytyy laskea uuden ja vanhan paneelin prosenttien erotus.
ruokasali = Huone(
    "Ruokasali",
...
# --------------------------------------------------
# HUONEIDEN NOLLAAMINEN
# --------------------------------------------------

# Palautetaan kaikkien huoneiden esineet alkuperäiseen tilanteeseen.
# Tätä käytetään, kun pelaaja aloittaa uuden pelin.
#
# Esineitä poistetaan huoneista sitä mukaa, kun pelaaja kerää niitä,
# joten huoneiden esinelistat täytyy palauttaa alkuperäisiksi
# uuden pelikerran alussa.
def nollaa_huoneet():
